...
class Fast2SMSService {
  ...
  static async sendDLTManualOTP(phone, otp) {
    ...
    try {
      ...
      const response = await axios.post(
        FAST2SMS_BASE_URL,
        `sender_id=${SENDER_ID}&message=${encodeURIComponent(message)}&template_id=${DLT_TEMPLATE_ID}&entity_id=${DLT_ENTITY_ID}&route=dlt_manual&numbers=${phone}`,
        {
          headers: {
            authorization: FAST2SMS_API_KEY,
            'Content-Type': 'application/x-www-form-urlencoded',
  ...
  static async checkBalance() {
    try {
      ...
      const response = await axios.get(
        `https://www.fast2sms.com/dev/wallet?authorization=${FAST2SMS_API_KEY}`
      );
