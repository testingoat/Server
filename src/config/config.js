...
export const authenticate = async (email, password) => {
    ...
    try {
        ...
        if (!user.password) {
            console.log('❌ Admin user has no password set:', email);
            return { success: false, message: 'Password not set for this admin user' };
        }
        ...
        console.log('🔍 Password comparison:', {
            provided: password ? '***' : 'missing',
            stored: user.password ? '***' : 'missing',
        ...
        if (user.password === password) {
            ...
            console.log('🔄 Returning auth result:', JSON.stringify(authResponse, null, 2));
